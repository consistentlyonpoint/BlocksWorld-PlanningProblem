...
class UnstackStack:
    def __init__(self):
        ...
        self.trapped_blocks = []
        #
        self.block_dll = BlockToMoveBlockTrapped()
    ...
    def solve(self, init_arr, goal_arr):
        ...
        # #####
        # self.to_move_blocks = [k for k, v in self.status_b.items() if v == "ToMove"]
        # # print("what is to_move_blocks? ", to_move_blocks)
        # self.trapped_blocks = [k for k, v in self.status_b.items() if v == "Trapped"]
        # # print("what is trapped_blocks? ", trapped_blocks)
        #####
        # while to_move_blocks and trapped_blocks:
        while self.to_move_blocks or self.trapped_blocks:
            # print("what is to_move_blocks? ", self.to_move_blocks)
            # print("what is trapped_blocks? ", self.trapped_blocks)
            if self.to_move_blocks:
                b = self.to_move_blocks[0]
                s_g_b = self.goal_sgx_dict[b]
                self.move_block(b, s_g_b)
                # self.to_move_blocks.pop(0)
            else:
                b = self.trapped_blocks[0]
                self.move_block(b, "Table")
                # self.trapped_blocks.pop(0)
            self.to_move_blocks = [k for k, v in self.status_b.items() if v == "ToMove"]
            self.trapped_blocks = [k for k, v in self.status_b.items() if v == "Trapped"]
    ...
    def status_block(self, b):
        ...
        if b != "Air" and b != "Table":
            if self.block_status_count_dict[b] < 15:
...
class BlockToMoveBlockTrapped:
    def __init__(self):
        self.block_head = None
        self.block_tail = None
        self.block_count = 0

    def add_block_back(self, block_data):
        new_block = BlockNode(block_data)
        #
        # if len(self.block_head) == 0:
        if self.block_head is None:
            self.block_head = new_block
            self.block_tail = new_block
        else:
            new_block.block_prev = self.block_tail
            self.block_tail.block_next = new_block
            self.block_tail = new_block
        #
        self.block_count += 1

    def add_block_front(self, block_data):
        new_block = BlockNode(block_data)
        #
        if self.block_head is None:
            self.block_head = new_block
            ...
            new_block.block_next = self.block_tail
            self.block_head.block_prev = new_block
            self.block_head = new_block
        #
        self.block_count += 1

    def remove_block(self, block_node):
        if block_node is not None and self.block_head is not None:
            if self.block_head == block_node:
                self.block_head = block_node.block_next
                self.block_head.block_prev = None
                #
                del block_node
                self.block_count -= 1
                return
            if self.block_tail == block_node:
                self.block_tail = block_node.block_prev
                self.block_tail.block_next = None
                #
                del block_node
                self.block_count -= 1
                return
            #
            block_node.block_next.block_prev = block_node.block_prev
            block_node.block_prev.block_next = block_node.block_next
            del block_node
            self.block_count -= 1
            return
        else:
    ...
    def block_find(self, block_data):
        search_block = self.block_head
        for b in range(self.block_count):
            if search_block.data == block_data:
                return search_block
